...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="Remove an object from a video using FlowDirector-style source-mask inpainting"
    )
    parser.add_argument(
        "--task",
        type=str,
        default="t2v-1.3B",
        choices=[task for task in WAN_CONFIGS.keys() if "t2v" in task],
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Background/removal target prompt, e.g. an empty grass field with no dog.")
    parser.add_argument(
        "--use_prompt_extend",
        action="store_true",
        default=False,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Object to remove in the source prompt; used to build the source attention mask.")
    parser.add_argument(
        "--target_words",
        type=str,
        default="",
...
def remove(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        logging.info("Creating WanT2V removal pipeline.")
        wan_t2v = wan.WanT2V(
            config=cfg,
        ...
        logging.info("Removing object from video ...")
        video = wan_t2v.edit(
            args.prompt,
    ...
    if rank == 0:
        if args.save_file is None:
            ...
            src_name = (args.source_words or "object").replace(" ", "_").replace("/", "_")
            args.save_file = f"videos/{formatted_time}_remove_{src_name}_n{args.n_avg}_w{args.worse_avg}_omega{args.omega}_s{args.base_seed}" + suffix
